...
!function (a) {
    ...
    if (!a.jstree) {
        ...
        j.setAttribute("role", "treeitem"), k = i.createElement("I"), k.className = "jstree-icon jstree-ocl", k.setAttribute("role", "presentation"), j.appendChild(k), k = i.createElement("A"), k.className = "jstree-anchor", k.setAttribute("href", "#"), k.setAttribute("tabindex", "-1"), l = i.createElement("I"), l.className = "jstree-icon jstree-themeicon", l.setAttribute("role", "presentation"), k.appendChild(l), j.appendChild(k), k = l = null, a.jstree = {
            ...
            plugin: function (b, c) {
                ...
                for (f.a_attr.id || (f.a_attr.id = f.id + "_anchor"), b.setAttribute("aria-selected", !!f.state.selected), b.setAttribute("aria-level", f.parents.length), b.setAttribute("aria-labelledby", f.a_attr.id),b.setAttribute("data-text", f.text),b.setAttribute("data-parents", getParents(f.parents,this.settings.core.data)), f.state.disabled && b.setAttribute("aria-disabled", !0), l = 0, m = f.children.length; m > l; l++)if (!q[f.children[l]].state.hidden) {
                    w = !0;
                    break
                }
    ...
    function getParents(i,core){
        ...
        if(i.length>1){
            $.each(i, function(k,v){
                if(v!='#'){
                    parentText = $.grep( core, function(o){return o.id==v;});
                    parents.push(parentText);
                }
            });

            var sort = Object.keys(parents).sort( function ( a, b ) {return b-a;});

            $.each(sort, function(key, sortId){
                parentSort.push(parents[sortId][0].text);
            });

            return parentSort.join(' - ');
        }else
            return '';
    }
});